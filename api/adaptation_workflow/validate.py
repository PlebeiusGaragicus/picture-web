...
def validate_character_artifact(path: Path) -> None:
    validate_character_file(path, require_variants=False)
...
def validate_character_list(book_root: Path, report: ValidationReport) -> None:
    ...
    for line in list_path.read_text().splitlines():
        ...
        character_file = book_root / "characters" / f"{slug}.md"
        if not validate_common_file(character_file, report):
            continue
        try:
            from adaptation_workflow.character_file import character_file_has_variants

            if character_file_has_variants(character_file):
                validate_character_file(character_file, require_variants=True)
            else:
                validate_character_stub(character_file)
